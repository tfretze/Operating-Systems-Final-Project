...
public class LRU {
    ...
    public LRU(ArrayList<Process> pList, int quantum){
    ...
    public void runLRU(){
        while(checkPageRemain()){
            ...
            if(!readyQueue.isEmpty()) {
                ...
                if(!currentProcess.checkForPageLRU(headPage)){
                    quantumCtr++;
                    currentProcess.loadPageLRU(headPage);
                    checkFault(currentProcess);
                    checkPageRemain(currentProcess);
                    checkQuantum(currentProcess);
                }else if(currentProcess.checkForPageLRU(headPage)) {
                    quantumCtr++;
                    currentProcess.sendToFront(headPage);
                    checkFault(currentProcess);
                    checkPageRemain(currentProcess);
                    checkQuantum(currentProcess);
                }
    ...
    public void checkFault(Process current){
        if(current.getTotalPages() > 0 && !current.checkForPageLRU(current.checkHead())){
